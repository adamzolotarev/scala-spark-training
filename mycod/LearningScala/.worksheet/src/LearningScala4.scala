object LearningScala4 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(293); 
  // Data structures
  
  // Tuples (Also really common with Spark!!) uuuuuuu
  // Immutable lists
...
// go into more depth - but you've got enough to get through this course for now.
 ...
 val myNumbers = (1 to 20).toList;System.out.println("""myNumbers  : List[Int] = """ + $show(myNumbers ));$skip(68); 
 for (number <- myNumbers) {
 	if (number % 3 == 0) println(number)
 	};$skip(81); val res$0 = 
 	myNumbers.filter(number => number % 3 == 0).map(number => println(number));System.out.println("""res0: List[Unit] = """ + $show(res$0))}
}
